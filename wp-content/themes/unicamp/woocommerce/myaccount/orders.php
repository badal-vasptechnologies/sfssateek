						...
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>"
							...
							<?php elseif ( 'order-number' === $column_id ) : ?>
								...
								<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>"
								   aria-label="<?php echo esc_attr( sprintf( __( 'View order number %s', 'unicamp' ), $order->get_order_number() ) ); ?>">
									<?php echo esc_html( _x( '#', 'hash before order number', 'unicamp' ) . $order->get_order_number() ); ?>
								</a>
							...
							<?php elseif ( 'order-total' === $column_id ) : ?>
								...
								echo wp_kses_post( sprintf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'unicamp' ), $order->get_formatted_order_total(), $item_count ) );
								?>
		...
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			<?php if ( 1 !== $current_page ) : ?>
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button<?php echo esc_attr( $wp_button_class ); ?>"
				   href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>"><?php esc_html_e( 'Previous', 'unicamp' ); ?></a>
			<?php endif; ?>
			...
			<?php if ( intval( $customer_orders->max_num_pages ) !== $current_page ) : ?>
				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button<?php echo esc_attr( $wp_button_class ); ?>"
				   href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ); ?>"><?php esc_html_e( 'Next', 'unicamp' ); ?></a>
			<?php endif; ?>
		</div>
	<?php endif; ?>
...
<?php else : ?>
	<div
		...
		<a class="woocommerce-Button button<?php echo esc_attr( $wp_button_class ); ?>"
		   href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Browse products', 'unicamp' ); ?>
		</a>
		<?php esc_html_e( 'No order has been made yet.', 'unicamp' ); ?>
	</div>
<?php endif; ?>
