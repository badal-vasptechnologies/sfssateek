...
class Unicamp_Walker_Nav_Menu extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
    {
        ...
        foreach ($children as $child) {
            ...
            if ($obj[0] === "ic_mega_menu") {
                $classes[] = apply_filters(
                    "insight_core_mega_menu_css_class",
                    "has-mega-menu",
                    $item,
                    $args,
                    $depth
        ...
        if ("ic_mega_menu" === $item->object) {
            $mega_menu_content_class = apply_filters(
                "insight_core_mega_menu_content_css_class",
                "mega-menu-content",
                $item,
                $args,
                $depth
            ...
            $output .=
                '<div class="' .
                esc_attr($mega_menu_content_class) .
                '">' .
                do_shortcode(
                    '[elementor-template id="' . $item->object_id . '"]'
                ) .
                "</div>";
        } else {
    ...
    function start_lvl(&$output, $depth = 0, $args = [])
    {
        ...
        $indent = str_repeat("\t", $depth);
        $output .= $indent . '<ul class="' . $class . '" data-badal=1>';
    }
}
