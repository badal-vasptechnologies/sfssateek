...
define( 'INSIGHT_CORE_VERSION', '2.7.0' );
...
if ( ! class_exists( 'InsightCore' ) ) {
	...
	function insight_core_check_updates() {
		$db_version = get_option( 'insight_core_db_version' );

		if ( empty( $db_version ) || version_compare( $db_version, '2.6.7', '<' ) ) {
			delete_transient( 'kirki_remote_url_contents' );
		}

		update_option( 'insight_core_db_version', INSIGHT_CORE_VERSION );
	}

	register_activation_hook( INSIGHT_CORE_FILE, 'insight_core_activation_hook' );

	function insight_core_plugins_update_completed( $upgrader_object, $options ) {
		InsightCore::write_log( 'Run updater hook' );
	...
	add_action( 'upgrader_process_complete', 'insight_core_plugins_update_completed', 10, 2 );
}
