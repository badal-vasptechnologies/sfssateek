...
class Library_Source extends Source_Base {
	...
	private static function request_library_data( $force_update = false ) {
		...
		$data = get_transient( self::LIBRARY_CACHE_KEY );
		...
		if ( $force_update || false === $data ) {
			...
			try {
				...
				if ( is_wp_error( $data ) || empty( $data ) || ! is_array( $data ) ) {
					throw new \Exception( 'Invalid response data' );
				}

				set_transient( self::LIBRARY_CACHE_KEY, $data, DAY_IN_SECONDS * 30 );
			} catch ( \Exception $e ) {
				/**
				 * If no data found then cache for 2 hrs to re-try again soon.
				 */
				set_transient( self::LIBRARY_CACHE_KEY, [], HOUR_IN_SECONDS * 2 );
	...
	private function prepare_template( array $template_data ) {
		...
		return [
			...
			'tags'          => $template_data['tags'] ?? array(),
			'url'           => $template_data['url'],
			'page_settings' => $template_data['page_settings'] ?? array(),
		];
	}
